...
bool Quadtree::Insert(Critter* gameObject) {
	...
	if (m_children == nullptr) {
		...
		if (m_objects[m_capacity - 1] == nullptr) {
			// we have at least 1 free space, so step through the array
			// and look for somewhere to insert the data
			for (int i = 0; i < m_capacity; i++) {
				if (m_objects[i] == nullptr) {
		...
		// if we coudldn't insert it in the objects array (ie, it was full),
		// then subdivide this node
		Subdivide();
	}
	// we couldn't insert the gameobject into this node, so find a child node
	// to insert into
	for (int i = 0; i < 4; i++) {
...
void Quadtree::Subdivide()
{
	...
	Vector2 qCentre{ m_boundary.m_centre.x - qSize.x,
	m_boundary.m_centre.y - qSize.y };
	m_children[TOP_LEFT] = new Quadtree(AABB(qCentre, qSize));
	qCentre = Vector2{ m_boundary.m_centre.x + qSize.x,
	m_boundary.m_centre.y - qSize.y };
	...
	if (m_objects != nullptr) {
		for (int i = 0; i < m_capacity; i++) {
			...
			// (dont delete it, just set it to null - the object still
			// needs to exist in the child node)
			m_objects[i] = nullptr;
		}
		// all of the objects that were stored in this node are now stored in
		// the child nodes, so delete the objects array.
		delete m_objects;
		m_objects = nullptr;
	}
}
...
void Quadtree::Draw()
{
	...
	DrawLine(
		m_boundary.m_centre.x - m_boundary.m_halfSize.x,
		m_boundary.m_centre.y + m_boundary.m_halfSize.y - 1,
		m_boundary.m_centre.x + m_boundary.m_halfSize.x,
		m_boundary.m_centre.y + m_boundary.m_halfSize.y - 1, RED);
	DrawLine(
		m_boundary.m_centre.x - m_boundary.m_halfSize.x + 1,
		m_boundary.m_centre.y + m_boundary.m_halfSize.y,
		m_boundary.m_centre.x - m_boundary.m_halfSize.x + 1,
		m_boundary.m_centre.y - m_boundary.m_halfSize.y, RED);
	DrawLine(
